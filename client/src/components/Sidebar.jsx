...
const Sidebar = ({ isMenuOpen, setIsMenuOpen }) => {
  const { chats, setSelectedChat, theme, setTheme, user, navigate,
   token, createNewChat,axios,setChats,fetchUserChats,setToken} = useAppContext();
  ...
  const deleteChat = async(e,chatId)=>{
             try{
              e.stopPropagation()
              const confirm = window.confirm("Are you want to delete chat")
               if(!confirm) return
  ...
  return (
    ...
    transition-all duration-500 max-md:absolute left-1 z-1 ${
    ...
    >
      ...
      <img
        ...
        alt=""
        className="w-full max-w-48 rounded-2xl  border border-black"
      />

      {/* New chat button */}
      <button onClick={createNewChat}
        className="flex justify-center items-center w-full
     py-2 mt-10 text-white bg-gradient-to-r from-[#A456F7] to-[#3D81F6]
     ...
     dark:border-white/20 rounded-md"
          ...
          .map((chat) => (
            ...
            >
              <div>
                ...
                <p
                  className="text-xs text-gray-500 dark:
                  text-[#B1A6C0]"
                >
              ...
              <img 
                ...
                className="hidden group-hover:block
                  w-6 h-6 cursor-pointer dark:invert"
                alt=""
      ...
      <div
        ...
        border-gray-300 dark:border-white/15 rounded-md cursor-pointer hover:scale-105
        transition-all"
      >
      ...
      <div className="flex items-center justify-between gap-2 p-3
                  mt-4 border border-gray-300 dark:border-white/15 rounded-md" >
      ...
      {/*User Account */} 
      <div 
        className="flex items-center gap-3 p-3 mt-4 border
        border-gray-300 dark:border-white/15 rounded-md cursor-pointer group"
      >
        <img
          src={assets.user_icon}
          className="w-7 dark:invert rounded-full"
          alt=""
        />
        ...
        {user && (
          <img onClick={logout}
            ...
            className="h-5 cursor-pointer
               hidden dark:invert group-hover:block"
          />
        )}
      </div>
